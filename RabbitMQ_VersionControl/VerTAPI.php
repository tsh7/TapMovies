<?php
	...
	$channel->queue_declare('Version_T_APILive', false, false, false, false);
	$msg = new AMQPMessage($newMessage, array('delivery_mode' => 2));//sends the sendVar to the RabbitMQ server
        $channel->basic_publish($msg, '', 'Version_T_APILive');//send message to API
        $channel->close();
        $connection->close();

        shell_exec("echo $PASSWORD | sudo -S ./toAPILive.sh $version");
        echo "Version sent";
