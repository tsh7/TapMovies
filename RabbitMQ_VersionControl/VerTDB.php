<?php
	...
	$channel->queue_declare('Version_T_DBLive', false, false, false, false);
	$msg = new AMQPMessage($newMessage, array('delivery_mode' => 2));//sends the sendVar to the RabbitMQ server
        $channel->basic_publish($msg, '', 'Version_T_DBLive');//send message to API
        $channel->close();
        $connection->close();

        shell_exec("./toDBLive.sh $version");
        echo "Version sent";
